...
-- selects all services and products in the salon
SELECT name, description FROM services
UNION
SELECT name, description FROM product;

-- selects all entries from services and products with a type label, combining them into one list
SELECT 'Service' AS type, name, description, duration AS 'Duration/Quantity', price AS 'Price/Cost'
FROM services
UNION ALL
SELECT 'Product' AS type, name, description, quantity AS 'Duration/Quantity', cost AS 'Price/Cost'
FROM product;

-- creates a distinct list of email domains used by both customers and employees
...
EXCEPT
SELECT customer_id, name FROM appointment JOIN customer ON customer.id = appointment.customer_id;
