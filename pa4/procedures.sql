...
DELIMITER $$
CREATE PROCEDURE product_details_by_name_sproc(IN product_name VARCHAR(255))
    BEGIN
        SELECT name, description, quantity, cost FROM product WHERE name = product_name;
    END $$

CREATE PROCEDURE total_income_from_payments_sproc(OUT total INT)
    BEGIN
        SELECT SUM(amount) INTO total FROM payment;
    END $$

CREATE PROCEDURE update_total_income_by_dates_sproc(INOUT total_income INT, IN start_date DATE, IN end_date DATE)
    BEGIN
...
CREATE PROCEDURE update_product_quantity_sproc(IN prod_id INT, IN quantity_minus INT)
    BEGIN
